...
export const useFetchPromotionProgramsOnTour = (tourId: string) => {
    const requestFn = async (): Promise<DiscountDetailDto> => {
        const response = await getPromoteProgramsApi().discountTourSearch(
            'root',
            tourId,
            DiscountOnType.EarlyBirdLastMinute,
        );
        return response.data;
    };
    return useQuery(['fetchPromotionProgramsOnTour', tourId], requestFn, {
        refetchOnWindowFocus: false,
        enabled: !!tourId,
    });
};

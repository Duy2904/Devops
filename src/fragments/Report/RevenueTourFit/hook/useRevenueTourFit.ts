...
export const useFetchRevenueTourFit = (request: AnyObject) => {
    const customKey = _.omit(request, 'pagination.total');
    return useQuery(
        ['fetchRevenueTourFit', JSON.stringify(customKey)],
...
export const useFetchProviderRevenue = () => {
    const requestFn = async () => {
        const response = await getRevenueApi().revenueReportGetSOProviders(TourNatureType.HNH, 'root');
        return (
    ...
    return useQuery(['FETCH_PROVIDERS_REVENUE'], requestFn, { refetchOnWindowFocus: false });
};

export const useSendCustomerRevenue = () => {
    const requestFn = async (groupIds: string[]) => {
        const fetchPromises = groupIds.map(id =>
            getRevenueApi().revenueReportGetCreatedBy(TourNatureType.HNH, id, 'root'),
        );
    ...
    return useMutation(['FETCH_SEND_CUSTOMER_REVENUE'], requestFn);
};

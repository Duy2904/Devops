...
import { TourSaleOrder, validateAmount } from '../Features';
...
export const TableData: React.FC<TableDataProps> = props => {
    const { data, form, recId, dataRec, isLoading, canChangeData, setSubmittable, setTravellerId } = props;
            ...
            render: (_, record: ReceivableVoucherLineDto) => (
                <>
                    <BaseInput
                        ...
                        disable={(dataRec?.status && dataRec?.status !== VoucherStatus.Received) || !canChangeData}
                    />
                </>
            ),
        },
    ...
    const handleDeleteRowData = (record: ReceivableVoucherLineDto) => {
        ...
        newArray.length == 0 && setSubmittable(false);
    };
    ...
    useEffect(() => {
        if (!isEmpty(itemReceivable.orderData) && data.length == 0) {
            ...
            setListSaleOrderSelected([
                { id: itemReceivable.orderData.tourScheduleId, saleOrderSelected: itemReceivable.orderData.id },
            ] as {
                id: string;
    ...
    return (
        <div className="px-5 my-3">
            {!recId && <CreateActionButton title={i18n.t('Thông tin chứng từ')} handleAdd={handleAdd} />}
            <Table
                columns={columns}
                dataSource={dataForm}
                rowKey="id"

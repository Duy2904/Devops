...
import { useGetRefundList } from '../../../hooks/useRefundList';
...
export const TableDataRF: React.FC = () => {
            ...
            render: (value: string, record: ReceivableVoucherDto) => {
                return (
                    <Can permissions={[MyPermissions.ReceivableVoucherView, MyPermissions.AgencyReceivableVoucherView]}>
                        <Link className="font-medium text-sm" to={`${rootPathsNew.refundViewDetail}/${record.id}`}>
                            {value}
                        </Link>
                    </Can>
                );

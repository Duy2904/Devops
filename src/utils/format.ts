...
class Format {
    ...
    public static formatNumberPhone = (value: string, char?: string) => {
    ...
    public static formatSearchDate(params: TableParams<AnyObject>, fromDate: Date | null, toDate: Date | null) {
        let tempFromDate;
        let tempToDate;

        if (isUndefined(params.fromDate)) {
            tempFromDate = fromDate;
        } else if (isNull(params.fromDate)) {
            tempFromDate = null;
        } else {
            tempFromDate = params.fromDate;
        }

        if (isUndefined(params.toDate)) {
            tempToDate = toDate;
        } else if (isNull(params.toDate)) {
            tempToDate = null;
        } else {
            tempToDate = params.toDate;
        }

        return { fromDate: tempFromDate, toDate: tempToDate };
    }
